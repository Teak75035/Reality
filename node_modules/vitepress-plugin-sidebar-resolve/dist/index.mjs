import { basename, resolve, join as join$1 } from 'node:path';
import { readdirSync, existsSync as existsSync$1, statSync as statSync$1, readFileSync as readFileSync$1 } from 'node:fs';
...
const readDirPaths = (sourceDir, ignoreList = []) => {
  const dirPaths = [];
  if (!existsSync$1(sourceDir)) return dirPaths;
  ...
  dirOrFilenames.forEach((dirOrFilename) => {
    ...
    if (!existsSync$1(secondDirPath)) return;
    if (!isSome(ignoreListAll, dirOrFilename) && statSync$1(secondDirPath).isDirectory()) {
...
const createSidebarItems$1 = (root, option, prefix = "/", onlyScannerRootMd = false) => {
  ...
  dirOrFilenames.forEach((dirOrFilename) => {
    ...
    if (!onlyScannerRootMd && statSync$1(filePath).isDirectory()) {
...
function createRewritesSidebar(rewrites = {}, option = {}, prefix = "/") {
  ...
  Object.entries(dirStructure).forEach(([dirName, dirOrFileInfo]) => {
    ...
    const dirRelativePath = join$1(path, dirPath);
    if (!existsSync$1(dirRelativePath)) return;
    if (!isSome(ignoreList, dirName) && !statSync$1(dirRelativePath).isDirectory()) return;
...
const createSidebarItems = (structure, root, option, prefix = "/", onlyScannerRootMd = false) => {
  ...
  entries.forEach(([dirOrFilename, dirOrFileInfo]) => {
    if (isSome(ignoreListAll, dirOrFilename)) return;
    const filePath = join$1(root, dirOrFilename);
    if (!existsSync$1(filePath)) return;
    const { index: indexStr, title, type, name } = resolveFileName(dirOrFilename, filePath, indexSeparator);
    const index = parseInt(indexStr, 10);
    if (!ignoreWarn && fileIndexPrefix && isIllegalIndex(index)) {
    ...
    if (!onlyScannerRootMd && statSync$1(filePath).isDirectory() && typeof dirOrFileInfo === "object") {
      const info = getInfoFromMarkdownDir(root, dirOrFilename);
      const mdTitle = titleFormMd ? info.title : "";
      const sidebarPrefix = (info.prefix && (prefixTransform?.(info.prefix) ?? info.prefix)) ?? "";
      const sidebarSuffix = (info.suffix && (suffixTransform?.(info.suffix) ?? info.suffix)) ?? "";
...
function VitePluginVitePressSidebarResolve(option = {}) {
  ...
  return {
    ...
    config(config) {
      ...
      const baseDir = path ? join$1(srcDir, path) : srcDir;
...
const setSideBar = (themeConfig, sidebar, type) => {
  ...
  if (type === "object") {
    ...
    themeConfig.sidebar = [
      ...sidebar,
      ...Object.prototype.toString.call(themeConfig.sidebar) === "[object Object]" ? logger.warn("\u81EA\u5B9A\u4E49 Sidebar \u5FC5\u987B\u662F\u6570\u7EC4\u5F62\u5F0F") : themeConfig.sidebar || []
    ];
  }
  logger.info("Injected Sidebar Data Successfully. \u6CE8\u5165\u4FA7\u8FB9\u680F\u6570\u636E\u6210\u529F!");
};

export { VitePluginVitePressSidebarResolve as default, getTitleFromMarkdown, isIllegalIndex, isSome };
