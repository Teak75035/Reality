...
const usePageState = () => {
  ...
  const isArchivesPage = Vue.computed(
    () => !!frontmatter.value.archivesPage || frontmatter.value.layout === "TkArchivesPage"
  );
  const isCataloguePage = Vue.computed(
    () => !!frontmatter.value.catalogue || frontmatter.value.layout === "TkCataloguePage"
  );
  const isArticleOverviewPage = Vue.computed(
    () => !!frontmatter.value.articleOverviewPage || frontmatter.value.layout === "TkArticleOverviewPage"
  );
  const isLoginUrl = Vue.computed(() => !!frontmatter.value.loginPage || frontmatter.value.layout === "TkLoginPage");
  const isRiskLinkPage = Vue.computed(
